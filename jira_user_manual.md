...
#### 3.1.2 1차 요청
##### 이슈 만들기에서 요청 유형을 "단순문의", "기술지원", "오류접수"로 선택하거나, 이슈 유형을 "Accu Common Issue"로 할 경우 해당 워크플로우로 티켓이 처리된다. 원격지원 요청은 기본적으로 원격지원 -> 현장지원에서 마무리되는 것을 전제로 하고 있지만 현장지원 이후 프로그램 변경이 필요한 경우 "요구사항 상세화 요청" 단계로 넘어갈 수 있다.(Accu Common Issue로 요청은 요청 유형이 자동으로 mapping되지 않으므로 사용을 자제하시기 바랍니다.)
