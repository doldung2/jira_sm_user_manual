...
### 3.1 전체 워크플로우
#### 요청 유형별로 "제안 및 검토지원요청", "원격지원요청", "요구사항 상세화요청" 으로 분기처리된다.
...
#### 3.1.1 제안 및 검토지원 요청
##### 이슈 만들기에서 요청 유형을 "프로젝트 기술 지원 및 자료요청"으로 선택하거나, 이슈 유형을 "Accu Support Issue"으로 할 경우 해당 워크플로우로 티켓이 수행된다.
...
#### 3.1.2 원격지원요청
##### 이슈 만들기에서 요청 유형을 "단순문의", "기술지원", "오류접수"로 선택하거나, 이슈 유형을 "Accu Common Issue"로 할 경우 해당 워크플로우로 티켓이 처리된다. 원격지원 요청은 기본적으로 원격지원 -> 현장지원에서 마무리되는 것을 전제로 하고 있지만 현장지원 이후 프로그램 변경이 필요한 경우 "요구사항 상세화 요청" 단계로 넘어갈 수 있다.
...
#### 3.1.3 요구사항 상세화 요청
##### 이슈 만들기에서 요청 유형을 "단순문의", "기술지원", "오류접수"로 선택하거나, 이슈 유형을 "Accu Common Issue"로 할 경우 해당 워크플로우로 티켓이 처리된다. 다만 "원격지원요청"과 차이점은 오류나 외부 접수 없이 자체로 개발을 진행하는 경우 해당 워크플로우로 수행된다. 이 경우 원격지원/현장지원 탭의 내용은 비어있어도 무관하며, 요건협의 및 이후 탭에 대해서는 내용이 기재되어야 한다. 

> 오류 및 외부문의가 없는 경우 일반적으로 수행되는 워크플로우이다.
> 화면 및 필드 내용은 앞의 "3.1.2 원격지원요청" 화면과 동일한 필드 구성으로 되어있다.
<kbd>
    <img src = ./images/user_016.png>
</kbd>